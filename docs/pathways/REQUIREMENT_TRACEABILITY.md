...
| `/discover` | **IMPLEMENTED (Phase 3: real entry point)** | Starts/resumes a real guest session via a server action (never a GET side effect), sets the HttpOnly guest cookie, redirects to `/discover/profile`. The allowlisted `interest` query param is carried forward as an editable GOALS-stage preselection, never a committed answer, and is removed from the URL once the session begins. |
| `/discover/profile` | **IMPLEMENTED (Phase 3)** | The real, conditional, server-backed 39-question Discovery Profile questionnaire -- see `docs/pathways/PHASE_STATUS.md` "Phase 3 evidence" for full detail. Private, `noindex`, session-owner-only. |
| `/discover/report` | **IMPLEMENTED (Phase 3: completion placeholder only)** | An honest, protected "Your Discovery Profile Is Complete" state. Explicitly does **not** generate or display any recommendation, ranking, score, or provider -- that is Phase 4/5, not started. |
...
## 12. Phase 3 — Discovery Profile requirement trace

Full narrative evidence is in `docs/pathways/PHASE_STATUS.md` "Phase 3 evidence"; this section traces the specific master-prompt-level requirements this phase implemented against their owning module and test reference.

| Requirement | Owning module | Test reference | Status |
|---|---|---|---|
| All 39 canonical questions (incl. `DISC_020A`) sourced only from `contracts/question-bank.json`, never a second registry | `src/lib/discovery/registry.ts` | `discovery-branching.test.ts` "loads the corrected 39-question canonical bank" | IMPLEMENTED |
| All 14 named branch predicates, one shared engine (client + server + tests) | `src/lib/discovery/branching.ts` (`BRANCH_PREDICATES`, `computeActiveFlow`) | `discovery-branching.test.ts` (25 tests, incl. required scenarios A-G) | IMPLEMENTED |
| Grade band derived only from DISC_002; age is context only | `gradeBandFromGrade()` | `discovery-branching.test.ts` "gradeBandFromGrade" (9 cases) | IMPLEMENTED |
| DISC_006/DISC_007 exactly-one-vs-multiple primary-reason logic, revalidated on change | `derivePrimaryReason()`, `validateCompletedProfile` | `discovery-branching.test.ts` "derivePrimaryReason" (5 tests); `discovery-validation.test.ts` "restricts single_from_previous... to the currently selected reasons" | IMPLEMENTED |
| `NOT_ANSWERED`/`UNKNOWN`/`NOT_APPLICABLE` kept as three distinct states; UNKNOWN satisfies a required question | `validation.ts` (`isAnswered`), `normalization.ts` | `discovery-validation.test.ts` "an explicit UNKNOWN satisfies a required question" | IMPLEMENTED |
| NONE/UNKNOWN/EXPLORING multi-select exclusivity, enforced server-side | `validation.ts` (`exclusiveValuesFor`) | `discovery-validation.test.ts` + `discovery-security.test.ts` (direct bypass attempts against the server function) | IMPLEMENTED |
| `family_priorities` capped at 1-3, never exactly 3 as a hardcoded requirement | `validation.ts` | `discovery-validation.test.ts` "rejects a 4th family priority"; `discovery-security.test.ts` "rejects a 4th family priority" | IMPLEMENTED |
| Free text (nickname ≤40, sport ≤60, parent context ≤750) sanitized, excluded from matching/AI/analytics | `validation.ts` (`sanitizeText`, `TEXT_MAX_LENGTH`) | `discovery-validation.test.ts` "sanitizeText" + length tests; `discovery-security.test.ts` "rejects an oversized nickname, sport name, and parent context" | IMPLEMENTED |
| Raw vs. effective answers kept separate; hidden/branch-inactive answers stop affecting effective state immediately, server-side | `normalization.ts` (`computeEffectiveAnswers`) | `discovery-validation.test.ts` "computeEffectiveAnswers"; `discovery-security.test.ts` "an inactive branch's answer never becomes effective, even if force-saved to raw storage" | IMPLEMENTED |
| Derived facts limited to the Specification-02 list (`grade_band`, `athletics_interest`, `homeschool_interest`, `home_or_online_interest`, `frequent_travel`, `foundation_concern`, `supplemental_need`, `ncaa_interest`) -- no invented derived fact | `normalization.ts` | `discovery-validation.test.ts` "derives grade_band, athletics_interest, and ncaa_interest facts" | IMPLEMENTED |
| Guest session reused (no parallel identity mechanism); HttpOnly cookie; POST-only session start | `src/server/discovery-draft.ts` (built on Phase 1's `src/server/session.ts`), `app/discover/actions.ts` | `discovery-draft.test.ts`; `tests/e2e/navigation.spec.ts` "the Discovery page is a real entry point..." (asserts the HttpOnly cookie) | IMPLEMENTED |
| Server-backed draft (no localStorage/sessionStorage/URL); smallest safe schema change | `discoverySession.draftAnswers` (jsonb) | `discovery-draft.test.ts`; `tests/e2e/discovery-profile.spec.ts` "localStorage/sessionStorage stay empty and the profile URL never contains raw answers" | IMPLEMENTED |
| Idempotent submission; one `ProfileRevision` per genuinely distinct submission | `discovery-draft.ts` (`computeSubmissionIdempotencyKey`, `submitDiscoveryProfile`), partial unique index | `discovery-draft.test.ts` "is idempotent" + "an intentional edit and resubmit creates a genuinely new revision"; `discovery-security.test.ts` concurrent-submission test | IMPLEMENTED |
| Server-authoritative 14-step submission sequence, transactional | `submitDiscoveryProfile()` | `discovery-draft.test.ts` "submits a valid profile transactionally" | IMPLEMENTED |
| No recommendation engine, no `contracts/rules.json` import, no AI, in Phase 3 | (absence verified) | code inspection (no import of `contracts/rules.json` anywhere under `src/lib/discovery/`, `src/server/discovery-draft.ts`, or `app/discover/**`); `discovery-profile.spec.ts` "the honest completion screen" (asserts no ranking/score/percent language) | IMPLEMENTED (verified absent) |
| Adaptive progress, no fixed "Question N of 39", no completion-time claim | `progress.ts`, `DiscoveryQuestionnaire.tsx` | manual/visual QA (`PHASE_STATUS.md` "Phase 3 evidence") | IMPLEMENTED |
| WCAG 2.2 AA target: real radio/checkbox semantics, visible focus, error association, `prefers-reduced-motion` | `QuestionField.tsx`, `QuestionField.module.css` | `tests/e2e/discovery-profile.spec.ts` "radio options are reachable and selectable by keyboard alone"; two accessibility bugs found and fixed this phase (see `PHASE_STATUS.md`) | IMPLEMENTED |
| Question-bank version recorded on every draft/completion, read from the loaded JSON | `registry.ts` (`QUESTION_BANK_VERSION`), `discoverySession.draftQuestionBankVersion`, `profileRevision` | `discovery-draft.test.ts` "starts a session carrying the question-bank version..." | IMPLEMENTED |
